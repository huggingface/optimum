...
When deploying a quantized model to a GPU, `CUDAExecutionProvider` cannot be used, and only models using static quantization can be executed on `TensorrtExecutionProvider`.
...
In this example, we can see all the costly MatMul operations are placed on the CUDA execution provider.
...
Once the required packages are installed, the following environment variables need to be set for ONNX Runtime to detect TensorRT installation:
```
