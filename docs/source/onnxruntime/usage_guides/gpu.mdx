...
Here are some performance results running with `CUDAExecutionProvider` when IOBinding has been turned on. We have tested input sequence length from 8 to 512, and generated output both with greedy search and beam search (`num_beam=5`):
...
And here is a summary for the saving time with different sequence lengths (from 8 to 512) and generation modes(greedy search and beam search) while using ONNX Runtime compared with PyTorch:
