...
TensorRT supports only models that use **static** quantization with **symmetric** weights and activations. Thus, for model quantized through Optimum to later be consumed by TensorRT, the following configuration needs to be passed when doing calibration for static quantization:
...
After performing static quantization, we can load the model into the `ORTModel` class using TensorRT as the execution provider. The fact that int8 operations are used needs to be specified to TensorRT, and ONNX Runtime graph optimization need to be disabled for the model to be consumed rather by TensorRT.
