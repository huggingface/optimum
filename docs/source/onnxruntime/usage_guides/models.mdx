...
are exported to the ONNX format, they are decomposed into three components that are later combined during inference:
- The text encoder component
- The U-NET component
- The VAE decoder component

Here is an example of how you can load a stable diffusion model, export it to the ONNX format and run inference using ONNX Runtime:
...
-stable_diffusion = StableDiffusionPipeline.from_pretrained(repo_id)
+stable_diffusion = ORTStableDiffusionPipeline.from_pretrained(repo_id, export=True)
...
![img](https://huggingface.co/datasets/optimum/documentation-images/resolve/main/onnxruntime/stable_diffusion_v1_5_ort.png)
