...
To load your PyTorch model and convert it to ONNX on-the-fly, simply pass `export=True` to the `from_pretrained` method.
After this step, don't forget to save your model using the `save_pretrained` method, to be able to load your ONNX model directly.

```python
stable_diffusion = ORTStableDiffusionPipeline.from_pretrained(model_id, export=True)

# Save exported the model
stable_diffusion.save_pretrained("a_local_path_for_convert_onnx_model")
```
