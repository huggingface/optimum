...
```python
import torch
from optimum.onnxruntime import ORTStableDiffusionPanoramaPipeline, ORTModelTiledVaeWrapper

model_id = "runwayml/stable-diffusion-v1-5"
pipeline = ORTStableDiffusionPanoramaPipeline.from_pretrained(model_id, revision="onnx")
...
image = pipeline(prompt=prompt, height=512, width=2048).images[0]
image.save("fantasy_landscape.png")
```
