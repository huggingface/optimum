...
With ONNX Runtime backend, [`ORTTrainer`] and [`ORTSeq2SeqTrainer`] take advantage of:

* Computation graph optimizations: node fusions, node eliminations...
...
runs use the Fused Adam Optimizer. The runs were performed on a single Nvidia A100 node with 8 GPUs.
