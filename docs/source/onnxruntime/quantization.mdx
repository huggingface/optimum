...
# Quantize the model
>>> model_quantized_path = quantizer.fit(
...
The `ORTQuantizer` currently doesn't support multi-file models, like `ORTModelForSeq2SeqLM`. If you want to quantize (statically/dynamically) a seq2seq model, you have to quantize each model individually using the `ORTQuantizer` class.
Below you can find a simple example on how to quantize a seq2seq model.
...
>>> model_path = onnx_model.model_save_dir
```
...
>>> encoder_quantizer = ORTQuantizer.from_pretrained(model_path, file_name="encoder_model.onnx")
...
# Quantize the model
>>> [q.fit(output_path=".",quantization_config=dqconfig) for q in quantizer]
```
