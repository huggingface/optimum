...
## Export and inference of Seq2Seq models

Sequence-to-sequence models, that generate a new sequence from an input, can also be used when running inference with ONNX Runtime. When Seq2Seq models are exported to the ONNX format, they are decomposed into three part that will be later combined during inference.
...
# Load the model from the hub and ONNX format
