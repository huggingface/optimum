...
Optimum Inference also includes methods to convert vanilla Transformers models to optimized ones via the `from_pretrained("{model_id}",from_transformers=True)` method.
...
The Optimum model classes, e.g. [`ORTModel`] are directly integrated with the [Hugging Face Model Hub](https://hf.co/models)) meaning you can not only 
