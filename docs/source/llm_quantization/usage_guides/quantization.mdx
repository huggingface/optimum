...
Optimum supports the `exllama_v2` backend for faster 4-bit GPTQ inference. In order to use these kernels, you need to have the entire model on GPUs.
...
Note that only 4-bit models are supported with ExLlamaV2 kernels for now. Furthermore, it is recommended to disable ExLlamaV2 kernels when you are finetuning your model with PEFT.
