...
You can easily une the `BetterTransformer` API using HuggingFace `optimum`, first install the dependencies as follows:
...
## Step 1: Load yout HuggingFace model

First, load your HuggingFace model using `transformers`. Make sure to download one of the models that is supported by the `BetterTransformer` API:
...
```
By default, `.transform` will overwrite your model, which means that your previous native model cannot be used anymore. If you want to keep it for some reasons, just add the flag `keep_original_model=True`!
```python
>>> from optimum.bettertransformer import BetterTransformer
...
`transformers` `pipeline` is also compatible with this integration and you can use `BetterTransformer` as an accelerator for your pipelines. Follow the snippet below for running a pipeline using the snippet below:
```python
...
```
If you want to run `pipeline` on a GPU device, run:
```python
>>> from optimum.pipelines import pipeline
