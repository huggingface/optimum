...
1. The input names.
2. The output names.
3. The dynamic axes. These refer to the inputs can be changed dynamically at runtime (e.g. a batch size or sequence length). All other axes will be treated as static, and hence fixed at runtime.
...
a specific model architecture, and follows the naming convention `ArchitectureNameOnnConfig`. For instance, the configuration which specifies the ONNX
