...
To train transformers on Habana's Gaudi processors, 🤗 Optimum provides a `IPUTrainer` that is very similar to the [🤗 Transformers trainer](https://huggingface.co/docs/transformers/main_classes/trainer). Here is a simple example:
...
+ from optimum.graphcore import IPUConfig, IPUTrainer, IPUTrainingArguments
  ...
  model = AutoModelForXxx.from_pretrained("Graphcore/bert-large-uncased")
...
To accelerate inference with ONNX Runtime, 🤗 Optimum uses _configuration objects_ to define parameters for optimization. These objects are then used to instantiate dedicated _optimizers_, _quantizers_, and _pruners_.
