...
Model configuration classes in 🤗 Transformers are not standardized. Although Transformers implements an `attribute_map` attribute that mitigate the issue to some extent, it does not make it easy to reason on common configuration attributes in the code.
