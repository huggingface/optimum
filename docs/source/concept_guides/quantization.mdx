...
data type in question. For examples, let's consider:

```
C = A x B
```

where `A` is a `m x n` matrix of `int8` values, and `B` is a `n x p` matrix of `int8` values.

Then for `i` in `[0, m - 1]` and `j` in `[0, p - 1]`, `C[i, j]` is an `int32` because it the sum of products of
`int8` values.
...
- `S` and `Z` are the quantization parameters
  ...
  - `Z` is called the zero-point, it is the `int8` value corresponding to the value `0` in the `float32` realm. This
  important to be be able to represent exactly the value `0` because it is used everywhere throughout machine learning
  models.
...
`(S, Z)` per element along one of the dimensions of a tensor. For example for a tensor of shape `[B, N, H, W]`, having
*per-channel* quantization parameters for the second dimension would result in having `N` pairs of `(S, Z)`. While this
...
results without too much work, it can be a bit slower than static quantization because the range is computed each time.
...
2. Try dynamic quantization, if it is fast enough stop here, otherwise continue to step 3.
3. Apply observers to your models in places where you want to quantize. It implies defining which quantization scheme to use.
