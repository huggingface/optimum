...
- Histogram: records a histogram of values along with min and max values, then chooses according to some criterion.
  ...
  the quantized data.
  - Percentile: the computed range is
  `[p% of the observed values are bigger than this one, p% of the observed value are smaller than this one]`,
  where `p` is the percentile value to use.
