...
class StableDiffusionPanoramaPipelineMixin(DiffusionPipelineMixin):
    ...
    def text2img(
        ...
        timestep_dtype = next(
            (input.type for input in self.unet.session.get_inputs() if input.name == "timestep"),
            "tensor(float)",
        )
    ...
    def inpaint(
        ...
        for i, t in enumerate(self.progress_bar(self.scheduler.timesteps)):
            ...
            for h_start, h_end, w_start, w_end in views:
                ...
                masked_latents_for_view = masked_image_latents[:, :, h_start:h_end, w_start:w_end]
        ...
        **kwargs,
        ...
        if "image" in kwargs or (
            ...
            if "mask_image" in kwargs or (
                len(args) > 2 and (isinstance(args[1], np.ndarray) or isinstance(args[1], PIL.Image.Image))
            ):
