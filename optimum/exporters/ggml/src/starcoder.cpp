// ref: https://github.com/ggerganov/ggml/blob/master/examples/starcoder/main.cpp
#include "ggml/ggml.h"
...
bool starcoder_model_load(const std::string & fname, starcoder_model & model, gpt_vocab & vocab) {
...

    {
        ...
        const int n_inner  = hparams.n_inner;
        ...
        ctx_size += n_layer*( n_inner*n_embd*ggml_type_sizef(wtype));         // c_mlp_fc_w
        ctx_size += n_layer*(        n_inner*ggml_type_sizef(GGML_TYPE_F32)); // c_mlp_fc_b

        ctx_size += n_layer*( n_inner*n_embd*ggml_type_sizef(wtype));         // c_mlp_proj_w
    ...
    // prepare memory for the weights
    {
        ...
        for (int i = 0; i < n_layer; ++i) {
            ...
            layer.c_mlp_fc_w    = ggml_new_tensor_2d(ctx, wtype,           n_embd, n_inner);
            layer.c_mlp_fc_b    = ggml_new_tensor_1d(ctx, GGML_TYPE_F32,   n_inner);

            layer.c_mlp_proj_w  = ggml_new_tensor_2d(ctx, wtype,           n_inner, n_embd);
