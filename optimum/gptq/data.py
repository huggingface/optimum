...
def prepare_examples(examples:List[Dict[str, Union[List[int],torch.LongTensor]]], pad_token_id, batch_size: int = 1):
    new_examples = []
...
def collate_data(blocks: List[Dict[str, List[torch.LongTensor]]], pad_token_id: int, contain_labels = False) -> Dict[str, torch.LongTensor]:
...
def get_examples(dataset_name, tokenizer, nsamples=128, seqlen=2048, seed=0):
    ...
    if dataset_name == "wikitext2":
        ...
        return get_c4(tokenizer= tokenizer,
                      nsamples= nsamples,
                      seqlen=seqlen)
    else:
