...
class RoFormerLayerBetterTransformer(BetterTransformerBaseLayer):
    def __init__(self, roformer_layer, config):
        r"""
        A simple conversion of the RoFormer Encoder layer to its `BetterTransformer` implementation.

        Args:
            roformer_layer (`torch.nn.Module`):
                The original RoFormer Layer where the weights needs to be retrieved.
        """
        super().__init__(config)
        # In_proj layer
        self.in_proj_weight = nn.Parameter(
        ...
        # Out proj layer
        self.out_proj_weight = roformer_layer.attention.output.dense.weight
        self.out_proj_bias = roformer_layer.attention.output.dense.bias

        # Linear layer 1
        self.linear1_weight = roformer_layer.intermediate.dense.weight
        self.linear1_bias = roformer_layer.intermediate.dense.bias

        # Linear layer 2
        self.linear2_weight = roformer_layer.output.dense.weight
        self.linear2_bias = roformer_layer.output.dense.bias

        # Layer norm 1
        self.norm1_eps = roformer_layer.attention.output.LayerNorm.eps
        self.norm1_weight = roformer_layer.attention.output.LayerNorm.weight
        self.norm1_bias = roformer_layer.attention.output.LayerNorm.bias

        # Layer norm 2
        self.norm2_eps = roformer_layer.output.LayerNorm.eps
        self.norm2_weight = roformer_layer.output.LayerNorm.weight
        self.norm2_bias = roformer_layer.output.LayerNorm.bias

        # Model hyper parameters
        self.num_heads = roformer_layer.attention.self.num_attention_heads
        self.embed_dim = roformer_layer.attention.self.all_head_size
