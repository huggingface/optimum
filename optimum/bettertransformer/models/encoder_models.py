...
class DPTViTLayerBetterTransformer(BetterTransformerBaseLayer):
    def __init__(self, dptvit_layer, config):
        r"""
        A simple conversion of the DPTViTLayer to its `BetterTransformer` implementation.

        Args:
            dptvit_layer (`torch.nn.Module`):
                The original `DPTViTLayer` where the weights needs to be retrieved.
        """
        super().__init__(config)
        # In_proj layer
        self.in_proj_weight = nn.Parameter(
            torch.cat(
                [
                    dptvit_layer.attention.attention.query.weight,
                    dptvit_layer.attention.attention.key.weight,
                    dptvit_layer.attention.attention.value.weight,
                ]
            )
        )
        self.in_proj_bias = nn.Parameter(
            torch.cat(
                [
                    dptvit_layer.attention.attention.query.bias,
                    dptvit_layer.attention.attention.key.bias,
                    dptvit_layer.attention.attention.value.bias,
                ]
            )
        )

        # Out proj layer
        self.out_proj_weight = dptvit_layer.attention.output.dense.weight
        self.out_proj_bias = dptvit_layer.attention.output.dense.bias

        # Linear layer 1
        self.linear1_weight = dptvit_layer.intermediate.dense.weight
        self.linear1_bias = dptvit_layer.intermediate.dense.bias

        # Linear layer 2
        self.linear2_weight = dptvit_layer.output.dense.weight
        self.linear2_bias = dptvit_layer.output.dense.bias

        # Layer norm 1
        self.norm1_eps = dptvit_layer.layernorm_before.eps
        self.norm1_weight = dptvit_layer.layernorm_before.weight
        self.norm1_bias = dptvit_layer.layernorm_before.bias

        # Layer norm 2
        self.norm2_eps = dptvit_layer.layernorm_after.eps
        self.norm2_weight = dptvit_layer.layernorm_after.weight
        self.norm2_bias = dptvit_layer.layernorm_after.bias

        # Model hyper parameters
        self.num_heads = dptvit_layer.attention.attention.num_attention_heads
        self.embed_dim = int(dptvit_layer.attention.attention.attention_head_size * self.num_heads)
