...
class NormalizedConfigManagerTest(unittest.TestCase):
    def test_layoutlm_is_registered(self):
        config_class = NormalizedConfigManager.get_normalized_config_class("layoutlm")
        self.assertIs(config_class, NormalizedTextConfig)

        normalized_config = config_class(LayoutLMConfig(hidden_size=48, num_attention_heads=6, num_hidden_layers=3))
