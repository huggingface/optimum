...
class NormalizedConfigManagerTest(unittest.TestCase):
    def test_flaubert_is_registered(self):
        config_class = NormalizedConfigManager.get_normalized_config_class("flaubert")
        self.assertIs(config_class, NormalizedTextConfig)

        normalized_config = config_class(FlaubertConfig(emb_dim=48, n_heads=6, n_layers=3))
