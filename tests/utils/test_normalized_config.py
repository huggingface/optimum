...
class NormalizedConfigManagerTest(unittest.TestCase):
    def test_levit_is_registered(self):
        config_class = NormalizedConfigManager.get_normalized_config_class("levit")
        self.assertIs(config_class, NormalizedVisionConfig)

    def test_levit_normalizes_expected_attributes(self):
        config = LevitConfig()
        normalized_config_class = NormalizedConfigManager.get_normalized_config_class("levit")
