...
class NormalizedConfigManagerTest(unittest.TestCase):
    def test_beit_is_registered(self):
        config_class = NormalizedConfigManager.get_normalized_config_class("beit")
        self.assertIs(config_class, NormalizedVisionConfig)

    def test_beit_normalizes_expected_attributes(self):
        config = BeitConfig()
        normalized_config_class = NormalizedConfigManager.get_normalized_config_class("beit")
