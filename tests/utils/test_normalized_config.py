...
class NormalizedConfigManagerTest(unittest.TestCase):
    def test_hiera_is_registered(self):
        config_class = NormalizedConfigManager.get_normalized_config_class("hiera")
        self.assertIs(config_class, NormalizedVisionConfig)

    def test_hiera_normalizes_expected_attributes(self):
        config = HieraConfig()
        normalized_config_class = NormalizedConfigManager.get_normalized_config_class("hiera")
