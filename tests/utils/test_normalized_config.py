...
from transformers import Owlv2Config, Pix2StructConfig
...
class NormalizedConfigManagerTest(unittest.TestCase):
    def test_owlv2_is_registered(self):
        config_class = NormalizedConfigManager.get_normalized_config_class("owlv2")
        self.assertIs(config_class, Pix2StructNormalizedTextConfig)

    def test_owlv2_normalizes_expected_attributes(self):
        # Same dual text/vision encoder shape as GroupViT/OwlViT/Pix2Struct. OwlV2's
        ...
        config = Owlv2Config()
        normalized_config_class = NormalizedConfigManager.get_normalized_config_class("owlv2")
